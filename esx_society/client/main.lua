...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
	...
	if ESX.PlayerData.job2 and ESX.PlayerData.job2.grade_name == 'boss' and 'society_' .. ESX.PlayerData.job2.name == society then
		UpdateSocietyMoney2HUDElement(money)
	end
end)
...
function EnableSocietyMoney2HUDElement()
	local societyMoney2HUDElementTpl = '<div><img src="' .. base64MoneyIcon .. '" style="width:20px; height:20px; vertical-align:middle;">&nbsp;{{money}}</div>'

	if ESX.GetConfig().EnableHud then
		ESX.UI.HUD.RegisterElement('society_money', 3, 0, societyMoney2HUDElementTpl, {
			money = 0
		})
	end
...
function DisableSocietyMoney2HUDElement()
	if ESX.GetConfig().EnableHud then
		ESX.UI.HUD.RemoveElement('society_money')
	end
...
function UpdateSocietyMoney2HUDElement(money)
	if ESX.GetConfig().EnableHud then
		ESX.UI.HUD.UpdateElement('society_money', {
			money = ESX.Math.GroupDigits(money)
		})
	...
	TriggerEvent('esx_society:setSocietyMoney2', money)
end
		...
		for i=1, #employees, 1 do
			...
			table.insert(elements.rows, {
				...
				cols = {
					employees[i].name,
					gradeLabel,
					'{{' .. _U('promote') .. '|promote}} {{' .. _U('fire') .. '|fire}}',"Emploi Principal"
				}
			})
		end
