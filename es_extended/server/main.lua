...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		...
		-- Org
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, org, org_gradeorg, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
			['@identifier'] = identifier
		}, function(result)
			...
			if ESX.DoesOrgExist(org, gradeorg) then
				...
				print(('[es_extended] [^3WARNING^7] Ignoring invalid org for %s [org: %s, gradeorg: %s]'):format(identifier, org, gradeorg))
				org, gradeorg = 'unemployed2', '0'
				orgObject, gradeorgObject = ESX.Org[org], ESX.Org[org].gradeorg[gradeorg]
			end
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.job2, userData.org, userData.loadout, userData.playerName, userData.coords)
